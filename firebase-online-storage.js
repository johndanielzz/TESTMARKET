(() => {
  ...
  function isPermissionDenied(error) {
    ...
    return code.indexOf("permission_denied") >= 0 || msg.indexOf("permission_denied") >= 0;
  }
  ...
  async function flushPending() {
    if (!db || !api || syncing || applyingRemote || !remoteWritable) return;
    if (!pendingClear && pending.size === 0) return;
    syncing = true;
    try {
      if (pendingClear) {
        await api.remove(api.ref(db, ROOT_PATH));
        pendingClear = false;
      }
      ...
      syncing = false;
      if (pendingClear || pending.size > 0) flushPending();
    }
  }
  ...
  const ready = (async () => {
    try {
      ...
      const firebaseDb = await import("https://www.gstatic.com/firebasejs/12.9.0/firebase-database.js");
      const firebaseAuth = await import("https://www.gstatic.com/firebasejs/12.9.0/firebase-auth.js");
      const firebaseAnalytics = await import("https://www.gstatic.com/firebasejs/12.9.0/firebase-analytics.js");
      ...
      try {
        ...
        if (supported) firebaseAnalytics.getAnalytics(app);
      } catch (_) {}

      await flushPending();
      window.dispatchEvent(new Event("mm-storage-hydrated"));
    } catch (_) {
      // Leave cache-only behavior when Firebase fails.
    }
  })();
