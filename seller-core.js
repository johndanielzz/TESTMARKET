(function () {
  ...
  function normalizeProduct(raw, idx) {
    const stock = Math.max(0, Math.floor(num(raw.stock || 0)));
    const explicitVisibility = (typeof raw.isVisible === "boolean") ? raw.isVisible : null;
    ...
    const needsRestock = Boolean(raw.needsRestock) || status === "soldout";
    const soldOutAt = status === "soldout"
      ? (raw.soldOutAt || raw.updatedAt || raw.createdAt || new Date().toISOString())
      : "";
    const isVisible = explicitVisibility == null ? status !== "hidden" : explicitVisibility;
    const key = str(raw.productKey || raw.sku || "");

    return {
      id: raw.id || ("PRD-" + Date.now() + "-" + idx),
      name: str(raw.name || raw.title || "Product"),
      description: str(raw.description || raw.desc || ""),
      price: num(raw.price || 0),
      image: str(raw.image || "https://via.placeholder.com/400x280?text=No+Image"),
      category: str(raw.category || "general"),
      stock: stock,
      location: str(raw.location || ""),
      seller: str(raw.seller || raw.sellerName || ""),
      sellerEmail: cleanEmail(raw.sellerEmail || ""),
      productKey: key || generateProductKey(raw.name || raw.title || "Product", raw.category || "general"),
      status: status,
