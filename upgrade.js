(function () {
  ...
  function initThemeToggle() {
    const saved = MMStorage.getItem('mm-theme');
    ...
    btn.addEventListener('click', function () {
      ...
      MMStorage.setItem('mm-theme', next);
      syncText();
    });
  ...
  function recordPageVisit() {
    try {
      ...
      const history = JSON.parse(MMStorage.getItem('recentPages') || '[]');
      const filtered = history.filter(function (item) {
        return item.path !== path;
      });
      filtered.unshift({ path: path, at: Date.now() });
      MMStorage.setItem('recentPages', JSON.stringify(filtered.slice(0, 8)));
    } catch (_err) {
      // no-op
    }
  }
